...
namespace Stash.Azure.Specifications.given_queries
{
    ...
    public class when_executing_less_than_equal_to_matches_none : with_int_indexer
    {
        ...
        {
            greaterThanTrackedGraph =
                new TrackedGraph(
                    new StoredGraph(
                        new InternalId(Guid.NewGuid()),
                        RegisteredGraph.GraphType,
                        AccessCondition.None, new PreservedMemoryStream("letspretendthisisserialiseddata".Select(_ => (byte)_).ToArray())),
            ...
            query = new LessThanEqualToQuery<int>(Subject.IndexDatabases[RegisteredIndexer.IndexName], RegisteredIndexer, 100);
        }
